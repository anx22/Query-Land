...
export const routeAlerts: ResourceRoute = async (store, method, pathname, _searchParams, body) => {
  const evaluateMatch = pathname.match(/^\/projects\/([^/]+)\/alerts\/evaluate$/);
  if (method === "POST" && evaluateMatch) {
    return json(201, { data: await store.evaluateAlerts(evaluateMatch[1]) });
  }

  const eventsMatch = pathname.match(/^\/projects\/([^/]+)\/alert-events$/);
  if (method === "GET" && eventsMatch) {
    return json(200, { data: await store.listAlertEvents(eventsMatch[1]) });
  }

  const ruleMatch = pathname.match(/^\/projects\/([^/]+)\/alert-rules\/([^/]+)$/);
  if (method === "DELETE" && ruleMatch) {
    return json(200, { data: await store.deleteAlertRule(ruleMatch[1], ruleMatch[2]) });
  }

  const rulesMatch = pathname.match(/^\/projects\/([^/]+)\/alert-rules$/);
  if (rulesMatch) {
    if (method === "GET") {
      return json(200, { data: await store.listAlertRules(rulesMatch[1]) });
    }
    if (method === "POST") {
      return json(201, { data: await store.createAlertRule(rulesMatch[1], asObject(body) as unknown as AlertRuleInput) });
    }
    return null;
  }
