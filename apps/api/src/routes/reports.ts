...
export const routeReports: ResourceRoute = async (store, method, pathname, searchParams, body) => {
  const runDueMatch = pathname.match(/^\/projects\/([^/]+)\/report-schedules\/run-due$/);
  if (method === "POST" && runDueMatch) {
    return json(202, { data: await store.runDueReportSchedules(runDueMatch[1]) });
  }

  const schedulesMatch = pathname.match(/^\/projects\/([^/]+)\/report-schedules$/);
  if (schedulesMatch) {
    if (method === "GET") {
      return json(200, { data: await store.listReportSchedules(schedulesMatch[1]) });
    }
    if (method === "POST") {
      return json(201, { data: await store.createReportSchedule(schedulesMatch[1], asObject(body) as unknown as ReportScheduleInput) });
    }
    return null;
  }
  ...
  if (collectionMatch) {
    if (method === "GET") {
      return json(200, { data: await store.listReports(collectionMatch[1]) });
    }
    if (method === "POST") {
      const type = asObject(body).type;
      return json(201, { data: await store.generateReport(collectionMatch[1], type as ReportType) });
    }
    return null;
  }
  ...
  if (method === "GET" && exportMatch) {
    ...
    return json(200, { data: await store.exportReport(exportMatch[1], format) });
  }
  ...
  if (method === "POST" && deliverMatch) {
    ...
    return json(201, { data: await store.deliverReport(deliverMatch[1], channel, target) });
  }

  const deliveriesMatch = pathname.match(/^\/reports\/([^/]+)\/deliveries$/);
  if (method === "GET" && deliveriesMatch) {
    return json(200, { data: await store.listReportDeliveries(deliveriesMatch[1]) });
  }

  const singleMatch = pathname.match(/^\/reports\/([^/]+)$/);
  if (method === "GET" && singleMatch) {
    return json(200, { data: await store.getReport(singleMatch[1]) });
  }
