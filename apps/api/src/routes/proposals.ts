...
export const routeProposals: ResourceRoute = (store, method, pathname, _searchParams, body): ApiResponse | null => {
  const transitionMatch = pathname.match(/^\/proposals\/([^/]+)\/transition$/);
  if (method === "POST" && transitionMatch) {
    const status = asObject(body).status;
    return json(200, { data: store.transitionProposal(transitionMatch[1], String(status) as ProposalStatus) });
  }

  const collectionMatch = pathname.match(/^\/projects\/([^/]+)\/proposals$/);
  if (collectionMatch) {
    if (method === "GET") {
      return json(200, { data: store.listProposals(collectionMatch[1]) });
    }
    if (method === "POST") {
      return json(201, { data: store.createProposal(collectionMatch[1], asObject(body) as unknown as CreateProposalInput) });
    }
    return null;
  }
