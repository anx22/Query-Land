...
export const routeBacklinks: ResourceRoute = async (store, method, pathname, searchParams) => {
  const importMatch = pathname.match(/^\/projects\/([^/]+)\/backlinks\/import$/);
  if (method === "POST" && importMatch) {
    return json(202, { data: await store.importBacklinks(importMatch[1]) });
  }
  ...
  if (method === "GET" && diffMatch) {
    return json(200, { data: await store.backlinkDiff(diffMatch[1]) });
  }

  const listMatch = pathname.match(/^\/projects\/([^/]+)\/backlinks$/);
  if (method === "GET" && listMatch) {
    const page = await store.listBacklinks(listMatch[1], paginationOptions(searchParams));
  ...
  if (method === "GET" && referringMatch) {
    return json(200, { data: await store.listReferringDomains(referringMatch[1]) });
  }

  const authorityMatch = pathname.match(/^\/projects\/([^/]+)\/authority$/);
  if (method === "GET" && authorityMatch) {
    return json(200, { data: await store.authoritySummary(authorityMatch[1]) });
  }

  const snapshotsMatch = pathname.match(/^\/projects\/([^/]+)\/backlink-snapshots$/);
  if (method === "GET" && snapshotsMatch) {
    return json(200, { data: await store.listBacklinkSnapshots(snapshotsMatch[1]) });
  }
