...
export const routeDiscoveredUrls: ResourceRoute = (store, method, pathname, searchParams, body): ApiResponse | null => {
  const listMatch = pathname.match(/^\/projects\/([^/]+)\/sites\/([^/]+)\/discovered-urls$/);
  if (listMatch) {
    if (method === "GET") {
      const page = store.listDiscoveredUrlsPage(listMatch[1], listMatch[2], paginationOptions(searchParams), {
    ...
    if (method === "POST") {
      ...
      const result = store.recordDiscoveredUrls(listMatch[1], listMatch[2], input.urls);
  ...
  if (method === "GET" && explorerMatch) {
    const page = store.listUrlExplorerRows(explorerMatch[1], explorerMatch[2], paginationOptions(searchParams), {
      status: enumQuery(searchParams, "status", fetchStatusFilter),
      source: enumQuery(searchParams, "source", sourceFilter)
    });
    return json(200, { data: page.data, meta: pageMeta(page) });
  ...
  if (fetchResultsMatch) {
    if (method === "GET") return json(200, { data: store.listFetchResults(fetchResultsMatch[1], fetchResultsMatch[2], fetchResultsMatch[3]) });
    if (method === "POST") return json(201, { data: store.recordFetchResult(fetchResultsMatch[1], fetchResultsMatch[2], fetchResultsMatch[3], recordFetchResultRequest(body)) });
    return null;
  }

  const indexabilityMatch = pathname.match(/^\/projects\/([^/]+)\/sites\/([^/]+)\/discovered-urls\/([^/]+)\/indexability$/);
  if (indexabilityMatch) {
    if (method === "GET") return json(200, { data: store.listIndexabilityAssessments(indexabilityMatch[1], indexabilityMatch[2], indexabilityMatch[3]) });
    if (method === "POST") return json(201, { data: store.recordIndexabilityAssessment(indexabilityMatch[1], indexabilityMatch[2], indexabilityMatch[3], recordIndexabilityRequest(body)) });
    return null;
  }
