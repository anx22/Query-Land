...
export const routeInternalLinks: ResourceRoute = async (store, method, pathname, searchParams, body) => {
  ...
  if (linksMatch) {
    if (method === "POST") {
      const edges = (body && typeof body === "object" && Array.isArray((body as { edges?: unknown }).edges))
        ...
        : [];
      const result = await store.recordInternalLinks(linksMatch[1], linksMatch[2], edges);
      return json(201, { data: result });
    }
    if (method === "GET") {
      const direction = enumQuery(searchParams, "direction", ["in", "out"]) ?? "out";
      const url = searchParams.get("url") ?? "";
      const page = await store.listInternalLinks(linksMatch[1], linksMatch[2], direction, url, paginationOptions(searchParams));
  ...
  if (method === "GET" && orphanMatch) {
    const page = await store.listOrphanUrls(orphanMatch[1], orphanMatch[2], paginationOptions(searchParams));
    return json(200, { data: page.data, meta: pageMeta(page) });
  }
