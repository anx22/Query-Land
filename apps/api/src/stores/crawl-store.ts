...
function mapCrawlPageSignal(crawlRunId: string, row: unknown): CrawlPageSignal {
  const record = row as { normalized_url: string; final_url: string; status_code: number | null; title: string | null; canonical_url: string | null; outgoing_links: string; onpage_signals?: string };
  ...
  let onPage: OnPageSignals = EMPTY_ON_PAGE_SIGNALS;
  try {
    const parsed = JSON.parse(record.onpage_signals ?? "{}");
    if (parsed && typeof parsed === "object") onPage = { ...EMPTY_ON_PAGE_SIGNALS, ...(parsed as Partial<OnPageSignals>) };
  } catch {
    // Corrupt/legacy JSON degrades to neutral on-page signals (no false positives).
  }
  return {
    crawlRunId,
    normalizedUrl: String(record.normalized_url),
...
class SQLiteCrawlStore implements CrawlStore {
  ...
  async recordCrawlPageSignals(projectId: string, siteId: string, crawlRunId: string, signals: Array<Omit<CrawlPageSignal, "crawlRunId">>): Promise<{ recorded: number }> {
    ...
    for (const signal of signals) {
      await this.db.prepare(`
        INSERT INTO crawl_page_signals (id, crawl_run_id, project_id, site_id, normalized_url, final_url, status_code, title, canonical_url, outgoing_links, onpage_signals, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(crawl_run_id, normalized_url) DO UPDATE SET
          ...
          onpage_signals = excluded.onpage_signals
      `).run(`signal-${randomUUID()}`, crawlRunId, projectId, siteId, signal.normalizedUrl, signal.finalUrl, signal.statusCode, signal.title, signal.canonicalUrl, JSON.stringify(signal.outgoingLinks ?? []), JSON.stringify(signal.onPage ?? EMPTY_ON_PAGE_SIGNALS), now);
    }
