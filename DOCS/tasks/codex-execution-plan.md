...
> **Status 2026-06-06:** **M0 und M1 inhaltlich abgeschlossen.** M1 liefert das Opportunity-Rückgrat (WP-1.2), den ersten Generator + binären Validierungsloop (WP-1.3), die reale Source Map (WP-1.1) sowie Opportunity Board + URL Dossier (WP-1.4). 74 API-Tests grün; Web kompiliert. Offene Follow-ups: `source_anchor` an generierte Opportunities hängen; Re-Check als Worker-Job; Crawler befüllt Linkgraph/connector_sync/PSI; AuthZ (WP-Z.1). Nächster Milestone: **M2 Keyword Core**.
