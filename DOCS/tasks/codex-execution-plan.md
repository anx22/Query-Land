...
**Fortschritt 2026-06-06:** WP-0.2 ist abgeschlossen. Der reproduzierbare Welle-1-Smoke liegt in `apps/api/test/app.test.ts`, läuft gegen den embedded API-Handler mit `sqlite::memory:` und ist Teil von `npm test`. WP-0.3 ist gestartet: Sitemap-Index-Auflösung, Redirect-Loop-Erkennung, Graceful-Shutdown-Handler und Worker-Smoke-Doku sind vorhanden; echte Site-Smokes bleiben auszuführen.
