...
> **Status 2026-06-06:** **M0, M1, M2, M3 und M4 abgeschlossen.** M4 liefert das Authority-Modul: GSC-Links-Stub (Klasse B, DEC-002), Snapshot-basiertes Backlink-Modell (Migration `010_backlinks.sql`), New/Lost-Diff, Authority-Intelligence-Metriken, sechs REST-Routen, UI `/backlinks` und drei read-only MCP-Tools (`get_authority_summary`, `list_referring_domains`, `get_backlink_changes`). Offene Follow-ups: echter GSC-OAuth-Provider (GAP-AUTH-001); Drittanbieter-Backlink-Quellen Klasse D (GAP-AUTH-002); Competitor-Gap-Analyse (GAP-AUTH-003). Nächster Milestone: **M5 Reporting**.
