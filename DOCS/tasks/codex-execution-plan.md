...
> **Status 2026-06-06:** **M0–M6 abgeschlossen.** M6 liefert das AI-Layer- und AEO-Modul: Domain-Typen `AiPrompt`/`AiAnswerSnapshot`/`AiVisibilityScore` + `computeAiVisibilityScore` (Klasse E, Signal), `AeoCheck`/`AeoResult`/`AeoAssessment` + pure `analyzeAeo` (Klasse A, Evidenz), `Proposal`/`ProposalKind`/`ProposalStatus`, Migration `012_ai_aeo.sql`, `ai-store` + `proposal-store`, `generateAeoOpportunities` (sechste harte Opportunity-Klasse `aeo`) im Umbrella-Generator, AI-Prompts-/Snapshot-/Visibility-/AEO-/Proposals-Routen, UI `/ai-visibility`, MCP read-only-Tools (`get_ai_visibility`, `list_proposals`) und review-gated MCP-Schreibtools (`create_dev_ticket`, `propose_fix_pr`). **Zusätzlich erledigt (2026-06-06):** PDF-Export (GAP-REPORT-001) — dependency-freier `reportToPdf` (PDF-1.4, Helvetica, ASCII-Transliteration) in `packages/domain-model/src/reports.ts`, verdrahtet durch `renderReportExport`, Report-Store, OpenAPI (`format=pdf`) und `/reports`-UI; MCP-Transport auf offizielles `@modelcontextprotocol/sdk` umgestellt (GAP-MCP-SDK geschlossen). Offene Follow-ups: echter LLM-Provider statt Stub (GAP-AI-001); Crawler-gelieferter Seiteninhalt für AEO (GAP-AI-002); echte Ticketing-/PR-Backend-Anbindung hinter MCP-Schreibtools (GAP-AI-003). **Verbleibende GAPs (GAP-REPORT-002/003, GAP-AUTH-001, GAP-AI-001, GAP-PERSIST-001) sind durch zwei strukturelle Faktoren geblockt: (a) keine Credentials in dieser Umgebung und (b) synchrone Store-Schicht (`node:sqlite` `DatabaseSync`) — blockende Netzwerk-`fetch`-Aufrufe können nicht in Store-Methoden laufen; echte Provider/Delivery und Persistenz-Migration gehören in den asynchronen Crawler/Worker-Pfad (Codex-Koordination) oder erfordern eine Async-Refaktorierung der Store-Schicht.** Verbleibendes Querschnitt-Paket vor produktivem Einsatz: **WP-Z.1 (AuthZ-minimal)**.
...
| **M3 Opportunity Engine + MCP** ✅ | 4 | Search-Performance-Intelligence (GSC-Stub), 5 Opportunity-Klassen, Prioritätsscore, Source-Map Pre-Merge-Gate, **MCP read-only** | jede Empfehlung mit Evidenz+Score+Validierungsmetrik; Agent beantwortet read-only Fragen |
...
| WP-1.1 Source Map real (minimal) ✅ | api/web | M | — | source-map.md — `source-map-store` upsert (url→template→repo) + `resolveSourceAnchor` (exakt/Präfix) + Deploy-Marker; Routen `POST /source-map`, `GET /source-map/resolve`, `/projects/{}/deploy-markers` + OpenAPI + Tests + Settings-UI. Follow-up: `source_anchor` an generierte Opportunities hängen |
...
| WP-1.4 Opportunity Board v0 ✅ + URL Dossier v0 ✅ | web | M | WP-1.2/1.3 | UX_FLOWS.md — `/content-opportunities` = Opportunity Board (priorisiert, Filter, Statuswechsel, Re-Validieren, Generieren); `/url-dossier` = eine URL als SEO-Objekt (Fetch/Indexability-Historie, Inlinks/Outlinks, Issues, Opportunities, Source-Anker) |
...
- **M3 (W4) ✅ abgeschlossen**: WP-3.1 Search-Performance-Intelligence ✅ (deterministischer GSC-Stub Klasse B, DEC-002; Migration 008; Query×Page-Matrix + reine Analyzer Striking-Distance/CTR-Gap/Kannibalisierung; `search-performance/{sync,intelligence}`) → WP-3.2 fünf harte Opportunity-Klassen + Prioritätsscore §6.4 ✅ (Generatoren low_hanging_keyword/money_page/cannibalization aus Search-Performance, internal_link_gap aus dem Linkgraph; idempotent; Umbrella `opportunities/generate`) → WP-3.3 Source-Map Pre-Merge-Gate ✅ (Migration 009 `pr_checks`; geänderte Repo-Pfade → betroffene Templates/URLs → verknüpfte offene Opportunities; passed/review_required/unmapped + Verlauf; Settings-UI) → WP-3.4 **MCP read-only** ✅ (dependency-freies `@seo-tool/mcp` mit stdio-JSON-RPC-Server + `createSeoMcpTools`: get_project_summary, get_url_dossier, list_opportunities, get_crawl_issues, explain_opportunity). UI: Opportunity Board synchronisiert Search-Performance, zeigt Intelligence und generiert alle Klassen. Offene Follow-ups: echter GSC-Provider statt Stub (DEC-002); PR-Check-Crawl-Diff im Worker; MCP-Anbindung an offizielles SDK/Transport.
- **M4 (W5) ✅ abgeschlossen**: GSC-Links-Stub (Klasse B, DEC-002) → Migration `010_backlinks.sql` (`backlink_snapshots` + `backlinks`) → `backlink-store` (`importBacklinks`, `listBacklinks`, `listReferringDomains`, `backlinkDiff`, `authoritySummary`, `listBacklinkSnapshots`) → Domain-Typen `Backlink/ReferringDomain/BacklinkSnapshot/BacklinkDiff/AuthoritySummary` + pure Funktionen `diffBacklinks`/`aggregateReferringDomains`/`summarizeAuthority` in `packages/domain-model` → Routen `POST /projects/{id}/backlinks/import`, `GET /projects/{id}/backlinks`, `/referring-domains`, `/backlinks/diff`, `/authority`, `/backlink-snapshots` → UI `/backlinks` → MCP read-only (`get_authority_summary`, `list_referring_domains`, `get_backlink_changes`). Architektur: `architecture/authority-backlinks.md`. Offene Follow-ups: echter GSC-OAuth-Provider (GAP-AUTH-001); Drittanbieter-Klasse-D-Quellen (GAP-AUTH-002); Competitor-Gap (GAP-AUTH-003).
- **M5 (W6) ✅ abgeschlossen**: Domain-Typen `Report/ReportSection/ReportDelivery/ReportSchedule/ReportExport/AlertRule/AlertEvent` + pure Funktionen `reportToCsv`/`reportToHtml`/`renderReportExport`/`compareAlert` in `packages/domain-model` → Migration `011_reporting.sql` (`reports`, `report_deliveries`, `report_schedules`, `alert_rules`, `alert_events`) → `report-store` (`generateReport` aggregiert Health/Opportunities/Visibility/Authority, `exportReport` CSV/HTML, `deliverReport` Stub-Kanal, `runDueSchedules` idempotent) + `alert-store` (`evaluateAlerts`, `listAlertEvents`) → zehn Report-Routen + sechs Alert-Routen → UI `/reports` → MCP read-only (`get_latest_report`, `list_alert_events`). Architektur: `architecture/reporting-alerts.md`. Offene Follow-ups: echter PDF-Renderer (GAP-REPORT-001); echte Email/Slack-Delivery (GAP-REPORT-002); worker-getriebener Cron (GAP-REPORT-003).
- **M6 (W7) ✅ abgeschlossen**: Domain-Typen `AiPrompt`/`AiAnswerSnapshot`/`AiVisibilityScore` + `computeAiVisibilityScore` (Klasse E) und `AeoCheck`/`AeoResult`/`AeoAssessment` + `analyzeAeo` (Klasse A) und `Proposal`/`ProposalKind`/`ProposalStatus` in `packages/domain-model` → Migration `012_ai_aeo.sql` (`ai_prompts`, `ai_answer_snapshots`, `aeo_assessments`, `proposals`) → `ai-store` (Prompts, deterministischer LLM-Snapshot-Stub Klasse E DEC-002, `getAiVisibilityScore`, `runAeoScan`) + `proposal-store` (create/list/transition) → `generateAeoOpportunities` (Klasse-A-Evidenz, sechste harte Opportunity-Klasse `aeo`) in den Umbrella-Generator eingebunden → AI-Prompts-/Snapshot-/Visibility-/AEO-/Proposals-Routen → UI `/ai-visibility` → MCP read-only (`get_ai_visibility`, `list_proposals`) + review-gated Schreibtools (`create_dev_ticket`, `propose_fix_pr`; erzeugen ausschließlich `proposals`-Zeilen im Status `proposed`, keine direkte Produktionsmutation). Architektur: `architecture/ai-layer-aeo.md`. Offene Follow-ups: echter LLM-Provider (GAP-AI-001); Crawler-gelieferter AEO-Inhalt (GAP-AI-002); externes Ticket-/PR-Backend (GAP-AI-003).
