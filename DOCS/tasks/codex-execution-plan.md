...
> **Status 2026-06-06:** **M0, M1, M2 und M3 abgeschlossen.** M3 liefert Search-Performance-Intelligence (WP-3.1), die fünf harten Opportunity-Klassen + Prioritätsscore (WP-3.2), das Source-Map Pre-Merge-Gate (WP-3.3) und die read-only MCP-Tool-Schicht (WP-3.4). 100 Tests grün (apps/api + packages/domain-model + services/crawler + services/mcp); Web kompiliert. Offene Follow-ups: echte Provider statt Stubs (DEC-002/GSC); Re-Check + PR-Check-Crawl-Diff + Linkgraph/connector_sync/PSI im Worker (mit Codex); MCP an offizielles SDK/Transport; AuthZ (WP-Z.1). Nächster Milestone: **M4 Authority**.
