...
> **Status 2026-06-06:** **M0, M1, M2, M3, M4 und M5 abgeschlossen.** M5 liefert das Reporting- und Alert-Modul: generisches `ReportSection`-Modell, dependency-freie Export-Pipeline CSV/HTML (`reportToCsv`/`reportToHtml`/`renderReportExport`), Delivery-Stub-Abstraktion (Email/Slack, DEC-002), idempotentes Schedule-/Automatisierungsmodell (`runDueSchedules`), Alert-Regel→Ereignis-Modell (`evaluateAlerts`/`compareAlert`), Migration `011_reporting.sql`, zehn Report-Routen, sechs Alert-Routen, UI `/reports` und zwei read-only MCP-Tools (`get_latest_report`, `list_alert_events`). Offene Follow-ups: echter PDF-Renderer (GAP-REPORT-001); echte Email/Slack-Delivery (GAP-REPORT-002); worker-getriebener Cron für Schedules (GAP-REPORT-003). Nächster Milestone: **M6 AI + MCP-Vollausbau**.
